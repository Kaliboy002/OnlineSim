        ...
        for number in numbers:
            ...
            bot.edit_message_text(
                ...
                text=(
                    ...
                    "⦿ در این بخش شما به صورت تصادفی میتوانید شماره مجازی دریافت و کد تایید آن را از دکمه پیام ها  بدست بیاورید. اما فراموش ن
                    
                    کنید که از شماره های این بخش همه کاربران قادر به استفاده بوده و شماره ممکن است قبلا توسط کاربر دیگری گرفته شده باشد.\n★<b> برای شماره تضمینی و همیشگی مخصوص شما، از بخش شماره مجازی خاص دریافت نماید</b>\n┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n\n"
                    "<b>⦿ بارگذاری کشور های آنلاین</b> :\n"
...
if engine.get_number_inbox(country['name'], number[1]):
    # Create the keyboard markup for the number
    keyboard = telebot.types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        telebot.types.InlineKeyboardButton("𖥸 پیـام های دریافتی", callback_data=f"msg&{country['name']}&{number[1]}"),
        telebot.types.InlineKeyboardButton("꩜ تازه سازی شماره", callback_data="new_phone_number"),
        telebot.types.InlineKeyboardButton("بررسی پروفایل این شماره", url=f"tg://resolve?phone=+{number[1]}")
    )
    
    # Update the prompt with the current status
    bot.edit_message_text(
        chat_id=message.chat.id,
        message_id=prompt.message_id,
        text=(
...
# Send failure message when no number is found
    ...
    # Update the prompt based on the current status
    bot.edit_message_text(
        chat_id=message.chat.id,
        message_id=prompt.message_id,
        text=(
        ...
        parse_mode="HTML",
        reply_markup=None  # No keyboard in case of failure
    )
